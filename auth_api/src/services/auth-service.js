...
import { tokensConfig } from '../../config.js';
...
const signup = async data => {
	try {
		...
		const accessToken = await jwt.sign(
			payload,
			tokensConfig.ACCESS_TOKEN_SECRET_KEY,
			tokensConfig.ACCESS_TOKEN_OPTIONS
		);

		const refreshToken = await jwt.sign(
			payload,
			tokensConfig.REFRESH_TOKEN_SECRET_KEY,
			tokensConfig.REFRESH_TOKEN_OPTIONS
		);
